...
| `LES-{册代码}-{课文代码}-{两位序号}` | S2先解析到同一课程目录的`_meta/evidence_manifest.json`、`_meta/lesson_plan_candidate.json`或`_meta/lesson_plan_lock.json`，G2后同时解析到`lesson.json`；同一ID不得跨目录，同一目录不得登记多个LES-ID | L2 |
...
| `PAPER-{卷代码}-{年份}` / `-Q{n}` | `work/knowledge/exams/papers/PAPER-{代码}-{年份}_*/`（卷代码表见 docs/architecture/试卷库组织约定.md；`GK-NC3-2016` 族为其历史别名） | L1 |
| `EXMAP-*` | `work/knowledge/exams/papers/` 内映射表按 `map_id` 查行 | L1 |
...
| `MM-{S环节}-{nn}` | `work/methodology/manuals/S{环节}-*手册.md` 条目 | 手册层 |
| `TW-*`（工作项） | 设计方案 §6.0 归属表 | — |
