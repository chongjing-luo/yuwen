...
11. 本试卷流水线受教材锁定门控制：81 张知识卡、28 份单元图谱和 5 份册级总表全部 `accepted`，并生成 `TEXTBOOK-LOCK` 后，才允许启动本规格的 G0/G1。
...
G0 不是当前立即执行的阶段。它只能在教材研究计划的 G-TB 锁定记录存在、版本和上游哈希通过校验后启动。
...
1. 教材 G-TB 锁定门已通过，且 G0—G6 全部通过，G7 明确作为后续独立阶段；
