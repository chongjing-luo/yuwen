#!/usr/bin/env python3
"""Validate the 2016--2024 practical-reading candidate batch."""
...
BATCH = ROOT / "work/knowledge/exams/workbench/kp_batches/practical_reading_2016_2024.jsonl"
REPORT = ROOT / "work/knowledge/_meta/practical_reading_2016_2024_kp_batch_validation_20260809.json"
...
def main() -> int:
    ...
    for row in rows:
        ...
        if row.get("response_form") != "practical_reading_response":
        ...
        if row.get("answer_candidate") is not None or row.get("answer_candidate_method") != "practical_response_not_auto_extracted":
            errors.append(f"{node}: practical answer was auto-extracted")
        ...
        if row.get("answer_candidate_status") != "practical_candidate_source":
            errors.append(f"{node}: unexpected candidate status")
        if row.get("manual_review_gate") != "practical_answer_and_evidence_review_required":
            errors.append(f"{node}: missing practical review gate")
    result = {
        "schema_version": "practical-reading-2016-2024-kp-batch-validation-0.1",
        ...
        "checks": {
            ...
            "response_gate": not any("auto-extracted" in error or "review gate" in error for error in errors),
