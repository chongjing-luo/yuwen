#!/usr/bin/env python3
"""Validate the 2016--2024 classical-memorization candidate batch."""
...
BATCH = ROOT / "work/knowledge/exams/workbench/kp_batches/classical_memorization_2016_2024.jsonl"
REPORT = ROOT / "work/knowledge/_meta/classical_memorization_2016_2024_kp_batch_validation_20260809.json"
...
def main() -> int:
    ...
    for row in rows:
        ...
        if row.get("response_form") != "fill_in_classical_memorization":
        ...
        if row.get("answer_candidate") is not None or row.get("answer_candidate_method") != "fill_in_answer_not_auto_extracted":
            errors.append(f"{node}: fill-in answer was auto-extracted")
        ...
        if row.get("answer_candidate_status") != "fill_in_candidate_source":
            errors.append(f"{node}: unexpected candidate status")
        if row.get("manual_review_gate") != "fill_in_answer_and_scoring_review_required":
            errors.append(f"{node}: missing fill-in review gate")
    result = {
        "schema_version": "classical-memorization-2016-2024-kp-batch-validation-0.1",
        ...
        "checks": {
            ...
            "fill_in_gate": not any("auto-extracted" in error or "review gate" in error for error in errors),
