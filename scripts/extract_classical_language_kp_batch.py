#!/usr/bin/env python3
"""Extract conservative candidate records for classical-language choice items.

Scope is limited to the 2009--2015 Sichuan papers' ancient-vocabulary and
ancient-function-word nodes.  This is a derived candidate layer: source text
and hashes are retained, while answer authority and textbook mapping remain
explicitly gated.
"""
...
TARGET_TYPES = {
    "ancient_vocab": ("文言实词语境释义", "结合语境解释文言实词"),
    "ancient_function_words": ("文言虚词意义和用法辨析", "结合语境辨析文言虚词意义和用法"),
}
...
def make_record(row: dict) -> dict:
    ...
    if upstream_status == "missing":
        status = (
            "candidate_text_present_authority_missing"
            if answer
            else ("candidate_source_without_answer_text_authority_missing" if analysis else "missing_analysis_source")
        )
...
def render(rows: list[dict]) -> str:
    ...
    for type_name in sorted(grouped):
        for row in sorted(grouped[type_name], key=lambda item: item["year"]):
            lines.append(
                f"| `{type_name}` | {row['year']} | `{row['exam_node_id']}` [[{row['prompt_source']}|题干]] | `{row['answer_candidate'] or 'N/A'}` | `{row['answer_candidate_status']}` | {row['candidate_atomic_exam_point']} | `{row['manual_review_gate']}` |"
            )
    ...
    lines += [
        ...
        "## 统计",
        "",
        f"- 总节点：{len(rows)}；显式候选答案：{counts.get('candidate_answer_present', 0)}；权威状态缺失但文本有答案：{counts.get('candidate_text_present_authority_missing', 0)}；无答案标记解析源：{counts.get('candidate_source_without_answer_text', 0)}；权威缺失且无显式答案：{counts.get('candidate_source_without_answer_text_authority_missing', 0)}。",
...
def main() -> int:
    ...
    print(json.dumps({"batch": "CLASSICAL-LANGUAGE-2009-2015", "record_count": len(rows), "status_counts": dict(Counter(row["answer_candidate_status"] for row in rows))}, ensure_ascii=False, indent=2))
    return 0
