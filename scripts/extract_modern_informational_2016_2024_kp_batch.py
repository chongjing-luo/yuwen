#!/usr/bin/env python3
"""Extract conservative candidate records for 2016--2024 informational reading."""
...
OUT_JSONL = OUT_DIR / "modern_informational_2016_2024.jsonl"
OUT_MD = OUT_DIR / "modern_informational_2016_2024.md"
...
def informational_evidence(text: str) -> tuple[str, list[str]]:
    ...
    markers = ("筛选并整合", "理解文中重要概念", "归纳内容要点", "概括中心意思", "推断", "本题考查学生")
    ...
    for path in sorted(SLICE_DIR.glob("GK-*-response_nodes_vertical_slice.jsonl")):
        for line in path.read_text(encoding="utf-8").splitlines():
            if not line.strip():
                continue
            row = json.loads(line)
            if 2016 <= row.get("year", 0) <= 2024 and row.get("question_type_l2") == "modern_reading_informational":
                rows.append(row)
...
def render(rows: list[dict]) -> str:
    lines = [
        "---", 'schema_version: "exam-kp-candidate-batch-0.1"', 'batch_id: "MODERN-INFORMATIONAL-2016-2024"',
    ...
    lines += ["", "## 统计", "", f"- 总节点：{len(rows)}；客观题解析候选源：{counts.get('objective_candidate_source', 0)}；缺少解析源：{counts.get('missing_analysis_source', 0)}。", "- `analysis_scope=question_segment_with_possible_related_context` 表示来源以题目段落定位，但解析段可能携带同组关联题上下文；答案仍需独立来源或 PDF 复核。", "- `answer_candidate` 全部保持空值，解析中出现“答案”字样不表示答案已核验。", "", "## 复核规则", "", "1. 逐页核对材料、题干、选项、题号、分值和 OCR/水印疑点。", "2. 将正确选项、原文证据、干扰项错因分栏登记；不能把解析结论直接当官方答案。", "3. 只有题文—答案/评分—教材 KP 三方证据闭合后，才允许升级映射等级。", "", "| 产物 | 路径 |", "|---|---|", "| JSONL | `work/knowledge/exams/workbench/kp_batches/modern_informational_2016_2024.jsonl` |", "| 本报告 | `work/knowledge/exams/workbench/kp_batches/modern_informational_2016_2024.md` |", "| 生成脚本 | `scripts/extract_modern_informational_2016_2024_kp_batch.py` |", ""]
    return "\n".join(lines)
...
def main() -> int:
    ...
    print(json.dumps({"batch": "MODERN-INFORMATIONAL-2016-2024", "record_count": len(rows), "year_counts": dict(actual), "status_counts": dict(Counter(row["answer_candidate_status"] for row in rows))}, ensure_ascii=False, indent=2))
    return 0
