#!/usr/bin/env python3
"""Extract conservative candidate records for informational-reading items."""
...
OUT_JSONL = OUT_DIR / "modern_informational_2009_2015.jsonl"
OUT_MD = OUT_DIR / "modern_informational_2009_2015.md"
...
def informational_evidence(text: str) -> tuple[str, list[str]]:
    if not text:
        return "", []
    lines = [line.strip() for line in text.splitlines() if line.strip()]
    markers = ("筛选并整合", "理解文中重要概念", "归纳内容要点", "概括中心意思", "推断", "能力层级")
...
def action_candidate(prompt: str) -> str:
    text = prompt or ""
    if "推断" in text:
        return "依据原文信息进行条件推断并检验边界"
    if "原因" in text:
        return "筛选并整合原文中的原因与证据"
    if "概念" in text or "理解" in text:
        return "结合语境理解重要概念和句子含义"
    return "对照原文筛选、整合并判断信息"
    ...
    for path in sorted(SLICE_DIR.glob("GK-SC-*-response_nodes_vertical_slice.jsonl")):
        for line in path.read_text(encoding="utf-8").splitlines():
            if not line.strip():
                continue
            row = json.loads(line)
            if 2009 <= row.get("year", 0) <= 2015 and row.get("question_type_l2") == "modern_reading_informational":
                rows.append(row)
...
def make_record(row: dict) -> dict:
    ...
    evidence, levels = informational_evidence(analysis)
    ...
    if upstream_status == "missing":
        ...
        status = "objective_candidate_source"
        gate = "objective_answer_and_evidence_review_required"
    else:
    ...
    return {
        ...
        "response_form": "objective_informational_reading",
        "analysis_scope": "question_segment_with_possible_related_context",
        "candidate_atomic_exam_point": "现代文信息筛选、概念理解、归纳与推断",
        "candidate_ability_action": action_candidate(row.get("prompt_excerpt")),
        "candidate_basis": "题型标签、题干任务和解析候选中的信息阅读考点描述；非正式知识点",
        ...
        "answer_candidate_method": "objective_answer_not_auto_extracted",
        ...
        "subskill_candidates": ["信息筛选", "概念理解", "归纳概括", "推断验证"],
        ...
        "na_reason": "客观题答案独立核验与教材KP双向证据尚未闭合。",
    }
...
def render(rows: list[dict]) -> str:
    ...
    lines += [
        ...
        "## 统计",
        "",
        f"- 总节点：{len(rows)}；客观题解析候选源：{counts.get('objective_candidate_source', 0)}；2013 年权威缺失：{counts.get('candidate_source_without_answer_text_authority_missing', 0)}。",
        ...
        "| 产物 | 路径 |",
        "|---|---|",
        "| JSONL | `work/knowledge/exams/workbench/kp_batches/modern_informational_2009_2015.jsonl` |",
        "| 本报告 | `work/knowledge/exams/workbench/kp_batches/modern_informational_2009_2015.md` |",
...
def main() -> int:
    ...
    print(json.dumps({"batch": "MODERN-INFORMATIONAL-2009-2015", "record_count": len(rows), "year_counts": dict(actual), "status_counts": dict(Counter(row["answer_candidate_status"] for row in rows))}, ensure_ascii=False, indent=2))
    return 0
