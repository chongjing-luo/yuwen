#!/usr/bin/env python3
"""Extract conservative candidate records for 2016--2024 literary reading."""
...
OUT_JSONL = OUT_DIR / "literary_reading_2016_2024.jsonl"
OUT_MD = OUT_DIR / "literary_reading_2016_2024.md"
...
def literary_evidence(text: str) -> tuple[str, list[str]]:
    ...
    markers = ("鉴赏文学作品", "分析作品结构", "归纳内容要点", "概括作品主题", "重要语句", "本题考查学生")
    ...
    for path in sorted(SLICE_DIR.glob("GK-*-response_nodes_vertical_slice.jsonl")):
        for line in path.read_text(encoding="utf-8").splitlines():
            if not line.strip():
                continue
            row = json.loads(line)
            if 2016 <= row.get("year", 0) <= 2024 and row.get("question_type_l2") == "literary_reading":
                rows.append(row)
...
def make_record(row: dict) -> dict:
    ...
    status = "literary_candidate_source" if analysis else "missing_analysis_source"
    gate = "literary_answer_and_scoring_review_required" if analysis else "answer_source_missing"
...
def render(rows: list[dict]) -> str:
    lines = [
        "---", 'schema_version: "exam-kp-candidate-batch-0.1"', 'batch_id: "LITERARY-READING-2016-2024"',
    ...
    lines += ["", "## 统计", "", f"- 总节点：{len(rows)}；文学阅读解析候选源：{counts.get('literary_candidate_source', 0)}；缺少解析源：{counts.get('missing_analysis_source', 0)}。", "- `analysis_scope=question_segment_with_possible_related_context` 表示题目段可能含同组关联上下文；解析结论不得跨小问复制。", "- `answer_candidate` 全部保持空值，解析中出现“答案”字样不表示答案已核验。", "", "## 复核规则", "", "1. 逐页核对文本、材料、题干、选项、分值和 OCR/水印疑点。", "2. 按小问分别登记文本证据、作答要点、评分点和解析来源；共享上下文不得替代小问证据。", "3. 只有题文—答案/评分—教材 KP 三方证据闭合后，才允许升级映射等级。", "", "| 产物 | 路径 |", "|---|---|", "| JSONL | `work/knowledge/exams/workbench/kp_batches/literary_reading_2016_2024.jsonl` |", "| 本报告 | `work/knowledge/exams/workbench/kp_batches/literary_reading_2016_2024.md` |", "| 生成脚本 | `scripts/extract_literary_reading_2016_2024_kp_batch.py` |", ""]
    return "\n".join(lines)
...
def main() -> int:
    ...
    print(json.dumps({"batch": "LITERARY-READING-2016-2024", "record_count": len(rows), "year_counts": dict(actual), "status_counts": dict(Counter(row["answer_candidate_status"] for row in rows))}, ensure_ascii=False, indent=2))
    return 0
