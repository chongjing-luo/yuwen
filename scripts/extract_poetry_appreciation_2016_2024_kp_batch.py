#!/usr/bin/env python3
"""Extract conservative candidate records for 2016--2024 poetry tasks."""
...
OUT_JSONL = OUT_DIR / "poetry_appreciation_2016_2024.jsonl"
OUT_MD = OUT_DIR / "poetry_appreciation_2016_2024.md"
...
def poetry_evidence(text: str) -> tuple[str, list[str]]:
    ...
    markers = (
        "古代诗歌阅读", "古代诗歌鉴赏", "鉴赏文学作品", "分析诗歌",
        "诗歌的理解和赏析", "本题考查学生", "本题属于综合考查", "本题主要考查", "本题要求赏析",
    )
...
def action_candidate(prompt: str) -> str:
    text = prompt or ""
    if any(term in text for term in ("情感", "心境", "志向", "态度")):
    ...
    for path in sorted(SLICE_DIR.glob("GK-*-response_nodes_vertical_slice.jsonl")):
        for line in path.read_text(encoding="utf-8").splitlines():
            if not line.strip():
                continue
            row = json.loads(line)
            if 2016 <= row.get("year", 0) <= 2024 and row.get("question_type_l2") == "poetry_appreciation":
                rows.append(row)
...
def make_record(row: dict) -> dict:
    ...
    status = "poetry_candidate_source" if analysis else "missing_analysis_source"
    gate = "poetry_answer_and_scoring_review_required" if analysis else "answer_source_missing"
...
def render(rows: list[dict]) -> str:
    lines = [
        "---", 'schema_version: "exam-kp-candidate-batch-0.1"', 'batch_id: "POETRY-APPRECIATION-2016-2024"',
    ...
    lines += ["", "## 统计", "", f"- 总节点：{len(rows)}；共享解析候选源：{counts.get('poetry_candidate_source', 0)}；缺少解析源：{counts.get('missing_analysis_source', 0)}。", "- `analysis_scope=shared_top_level_analysis_segment` 表示同一解析段可能服务同题多小问；不能把段内任一结论直接归给单个小问。", "- `answer_candidate` 全部保持空值；解析中出现“答案”字样也不表示答案已核验。", "", "## 复核规则", "", "1. 逐页核对诗文、设问、分值、材料边界和 OCR/水印疑点。", "2. 按小问分别登记诗句证据、作答要点、评分点和解析来源；共享解析段不得跨小问复制结论。", "3. 只有题文—答案/评分—教材 KP 三方证据闭合后，才允许升级映射等级。", "", "| 产物 | 路径 |", "|---|---|", "| JSONL | `work/knowledge/exams/workbench/kp_batches/poetry_appreciation_2016_2024.jsonl` |", "| 本报告 | `work/knowledge/exams/workbench/kp_batches/poetry_appreciation_2016_2024.md` |", "| 生成脚本 | `scripts/extract_poetry_appreciation_2016_2024_kp_batch.py` |", ""]
    return "\n".join(lines)
...
def main() -> int:
    ...
    print(json.dumps({"batch": "POETRY-APPRECIATION-2016-2024", "record_count": len(rows), "year_counts": dict(actual), "status_counts": dict(Counter(row["answer_candidate_status"] for row in rows))}, ensure_ascii=False, indent=2))
    return 0
