#!/usr/bin/env python3
"""Validate the literary-reading candidate batch."""
...
BATCH = ROOT / "work/knowledge/exams/workbench/kp_batches/literary_reading_2009_2015.jsonl"
REPORT = ROOT / "work/knowledge/_meta/literary_reading_kp_batch_validation_20260809.json"
...
def main() -> int:
    ...
    if len(rows) != 28:
        errors.append(f"row_count={len(rows)} expected=28")
    ...
    result = {
        "schema_version": "literary-reading-kp-batch-validation-0.1",
        "result": "passed" if not errors else "failed",
        "row_count": len(rows),
        "year_counts": dict(actual),
        "errors": errors,
        "checks": {
            "coverage": len(rows) == 28 and dict(actual) == EXPECTED_COUNTS,
