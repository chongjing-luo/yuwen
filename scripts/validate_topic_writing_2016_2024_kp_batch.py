#!/usr/bin/env python3
"""Validate the 2016--2024 topic-writing candidate batch."""
...
BATCH = ROOT / "work/knowledge/exams/workbench/kp_batches/topic_writing_2016_2024.jsonl"
REPORT = ROOT / "work/knowledge/_meta/topic_writing_2016_2024_kp_batch_validation_20260809.json"
...
def main() -> int:
    ...
    for row in rows:
        ...
        if row.get("analysis_scope") != "top_level_exam_segment_with_possible_unrelated_context":
            errors.append(f"{node}: writing scope not declared")
        ...
        if row.get("answer_candidate_status") != "writing_candidate_source":
            errors.append(f"{node}: unexpected candidate status")
        if row.get("manual_review_gate") != "writing_answer_and_scoring_review_required":
            errors.append(f"{node}: missing writing review gate")
    result = {
        "schema_version": "topic-writing-2016-2024-kp-batch-validation-0.1",
        "result": "passed" if not errors else "failed", "row_count": len(rows), "year_counts": dict(actual), "errors": errors,
        "checks": {
            ...
            "writing_gate": not any("auto-extracted" in error or "review gate" in error for error in errors),
            "scope_gate": not any("scope not declared" in error for error in errors),
