...
function masterMarkdown() {
  const lines = ["---", "document_type: teaching_master", `version: ${lesson.version}`, "status: no_image_implementation_candidate", `logical_pages: ${lesson.target_logical_pages}`, `physical_states: ${lesson.target_pages}`, `natural_minutes: ${lesson.target_natural_minutes}`, "---", "", "# 《氓》V6.6 V5教学母版", "", "## 课程脊柱", "", "广泛回忆爱情与婚姻文学—教师引用现场作品后揭题—建立《诗经》最低文化坐标—完整听诗—沿六章逐句读懂并逐章形成个人章意—个人六章长卷—把不幸还原为生活并完成个人末答—质询婚姻悲剧的原因并完成个人末答—从原诗提出良好共同生活的支点—文化、字词、诗法检索修复—完整终读。", "", "## 三个全文问题", "", ...lesson.three_questions.map((value, index) => `${index + 1}. ${value}`), "", "## 逐页结构", "", "| 逻辑页 | ID | 分钟 | 物理状态 | 唯一意义 | 学生动作 | 产物 | 首次后用 |", "|---:|---|---:|---:|---|---|---|---|"];
